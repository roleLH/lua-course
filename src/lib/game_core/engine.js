// import GCore from "../../core";
  ...
  const font_min_y = font_ys.reduce((a, b) => Math.min(a, b));
  const font_max_y = font_ys.reduce((a, b) => Math.max(a, b));
...
const sfx_update = function() {
  for(let i = 0; i < 4; i++) {
    ...
    const next_note_full_index = Math.ceil(state_time / period);
  ...
  if(time == 0) {
    ...
    for(let i = 0; i < rcn.music_track_count; i++) {
      if(ram[mus_index + i] & 0x40) {
        ...
        max_time = Math.max(max_time, period * 32);
        tracks.push({
          sound: track_sound,
          period: period * 32,
        });
    ...
    for(let i = 0; i < tracks.length; i++) {
      GEngine.sfx(tracks[i].sound, i, 0, 32, Math.floor(max_time / tracks[i].period));
    }
...
const GEngine = {
  init: (rcn) => {
    rcn_ram_size = rcn.ram_size;
    rcn_mem_palette_offset = rcn.mem_palette_offset;
    rcn_mem_sound_offset = rcn.mem_sound_offset;
    rcn_mem_music_offset = rcn.mem_music_offset;
    rcn_mem_gamepad_offset = rcn.mem_gamepad_offset;
    rcn_mem_musicstate_offset = rcn.mem_musicstate_offset;
    rcn_mem_soundstate_offset = rcn.mem_soundstate_offset;
    rcn_mem_soundreg_offset = rcn.mem_soundreg_offset;
    rcn_mem_soundreg_size = rcn.mem_soundreg_size;
    rcn_mem_screen_offset = rcn.mem_screen_offset;
    rcn_mem_screen_size = rcn.mem_screen_size;
  ...
  cls : function(c = 0) { // Default color is 0
  ...
  spr : function(n, x, y, ow = 1.0, oh = 1.0, fx = false, fy = false) {
    x -= cam_x();
    y -= cam_y();

    // Clip
    const iw = Math.max(0, -x / 8);
    const ih = Math.max(0, -y / 8);
    const w = Math.min(ow, (128 - x) / 8);
    const h = Math.min(oh, (128 - y) / 8);
  ...
  map : function(celx, cely, sx, sy, celw, celh, layer = 0xff) {
    for(let x = 0; x < celw; x++) {
      for(let y = 0; y < celh; y++) {
        GEngine.spr(GEngine.mget(celx + x, cely + y), sx + (x << 3), sy + (y << 3));
      }
    }
  },
  ...
  line : function(x0, y0, x1, y1, c) {
    ...
    const dx = Math.floor(x1 - x0);
    const dy = Math.floor(y1 - y0);
    if(dx == 0 && dy == 0) {
  ...
  print : function(x, y, text, c) {
    ...
    for(let i = 0; i < text.length; i++) {
      ...
      x += advance;
      max_width = Math.max(max_width, x - ox);
    }
    return max_width;
  },
  ...
  // sound API
  sfx : function(n, channel = -1, offset = 0, length = 32, loops = 1) {
    if(channel < 0) {
  ...
  // input API
  btn : function(i, p = 0) { // First player by default
  ...
  // // debug API
  // debug : function(msg) {
  //   GCore.emit("engine_debug", {
  //     msg
  //   });
  // },

  read : (offset, size) => {
    return ram.slice(offset, size);
  },
  write : (bytes, offset) => {
